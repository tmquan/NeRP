...
class TensorboardGenerativeModelImageProjector(Callback):
    ...
    def on_epoch_end(self, 
        ...
        cameras = pl_module.varcams

        # generate images
        with torch.no_grad():
            pl_module.eval()
            viewed_, mapped_ = pl_module(image3d=image3d, cameras=cameras, factor=None)
            pl_module.train()
        ...
        viz = torch.cat([image3d[...,64], mapped_[:,:1,...,64], 
                         mapped_[:,[1],...,64] / (pl_module.gen[0].scaler), 
